...
function LeaveActionContent() {
    ...
    useEffect(() => {
        ...
        const fetchLeave = async () => {
            try {
                const res = await fetch(`/api/admin/Leave/get-single?id=${id}`)
                ...
                if (data.success) {
                    ...
                    setErrorMsg(data.message || 'Leave request not found')
                }
            } catch (err) {
                setErrorMsg('Failed to load leave details')
            } finally {
                setLoading(false)
            }
        }
    ...
    const handleAction = async (actionSubmit: 'approved' | 'rejected', reason: string) => {
        setProcessing(true)
        try {
            const res = await fetch('/api/admin/Leave/quick-action', {
                method: 'POST',
            ...
            const data = await res.json()
            if (data.success) {
                setLeaveData(prev => ({ ...prev, status: actionSubmit }))
                setComplete(true)
            } else {
                setErrorMsg(data.message || 'Failed to update leave request')
            }
        } catch (err) {
    ...
    if (complete || leaveData.status !== 'pending') {
        const isApprove = leaveData.status === 'approved'
        return (
            <div className="min-h-screen bg-[#0f172a] text-white flex flex-col items-center justify-center p-6">
                <div className="bg-[#1e293b] p-8 rounded-2xl border border-gray-700 max-w-md w-full text-center shadow-2xl">
                    ...
                    <div className="mx-auto w-20 h-20 rounded-full overflow-hidden border-2 border-gray-500 mb-4 shadow-lg">
                        {leaveData.profilePicture ? (
                            <img src={leaveData.profilePicture} alt="Profile" className="w-full h-full object-cover" />
                        ) : (
                            <div className="w-full h-full bg-blue-600 flex items-center justify-center text-3xl font-bold text-white">
                                {leaveData.userName?.[0]?.toUpperCase()}
                            </div>
                        )}
                    </div>

                    <h2 className="text-2xl font-bold mb-1">{leaveData.userName}</h2>
                    <p className="text-gray-400 text-sm mb-6">{leaveData.designation}</p>

                    <div className="bg-[#0f172a] p-4 rounded-xl mb-6 text-left border border-gray-800">
                        <p className="text-sm text-gray-400 mb-1">Leave Duration</p>
                        <p className="font-semibold">{leaveData.totalDays} Day(s)</p>
                        <div className="h-px bg-gray-800 my-2"></div>
                        <p className="text-xs text-gray-400">From: <span className="text-gray-200">{new Date(leaveData.fromDate).toDateString()}</span></p>
                        <p className="text-xs text-gray-400">To: <span className="text-gray-200">{new Date(leaveData.toDate).toDateString()}</span></p>
                    </div>
                    ...
                    <p className="text-xs text-gray-500 mt-6">Email notification has been sent to the user.</p>
                </div>
            </div >
        )
    }
    ...
    return (
        <div className="min-h-screen bg-[#0f172a] text-white flex flex-col items-center justify-center p-6">
            <div className="bg-[#1e293b] p-8 rounded-2xl border border-gray-700 max-w-md w-full shadow-2xl">
                ...
                <h2 className="text-2xl font-bold mb-2 text-center text-red-400">Reject Leave Request</h2>
                <p className="text-gray-400 text-sm text-center mb-6">Provide a reason for rejecting the leave request from <b>{leaveData.userName}</b>.</p>

                <div className="bg-[#0f172a] p-4 rounded-xl mb-6">
                    <p className="text-xs text-gray-400">Requested Days: <span className="text-white font-bold">{leaveData.totalDays} Day(s)</span></p>
                    <p className="text-xs text-gray-400 mt-1">Reason: <span className="text-white">{leaveData.reason}</span></p>
                </div>
                ...
                <div className="mb-6">
                    ...
                    <textarea
                        ...
                        placeholder="Please specify why this leave is rejected..."
                    ></textarea>
                </div>
...
export default function LeaveActionPage() {
    return (
        <Suspense fallback={<div className="min-h-screen bg-[#0f172a] flex items-center justify-center"><div className="animate-spin rounded-full h-10 w-10 border-b-2 border-blue-500"></div></div>}>
            <LeaveActionContent />
        </Suspense>
    )
}
