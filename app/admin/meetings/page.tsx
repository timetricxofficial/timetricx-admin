...
export default function AdminMeetingsPage() {
  ...
  /* ---------------- AUTO DELETE MEETING (Timer) ---------------- */
  const autoDeleteMeeting = async (id: string) => {
    try {
      const res = await fetch(`/api/admin/meetings/${id}`, {
        method: 'DELETE'
      })
      ...
      if (res.ok) {
        // Remove from local state without showing toast
        setMeetings(prev => prev.filter(m => m._id !== id))
      }
    } catch (err) {
      console.error('Auto delete meeting error:', err)
    }
  }
